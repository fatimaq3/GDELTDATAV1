...
def fetch_latest_url():
    # Try yesterday's file — if not available yet, skip gracefully
    ...
    if resp.status_code == 404:
        log.warning(f"File not available yet for {date_str} — skipping run")
...
def process(df, event_date):
    ...
    df["sentiment"] = pd.to_numeric(df["AvgTone"], errors="coerce").fillna(0).apply(lambda t: "Positive" if t > 2 else ("Negative" if t < -2 else "Neutral"))
...
def run():
    ...
    try:
        ...
        if url is None:
            log.info("No file available for today — pipeline skipped cleanly")
            return
