...
# ══════════════════════════════════════════════════════════════════════════
# PRIORITY SOURCES — these get priority score 0 (highest)
# ══════════════════════════════════════════════════════════════════════════
PRIORITY_SOURCES = {
    # Arabic news
    "bbc.com/arabic", "arabic.bbc.co.uk",
    "aljazeera.net", "aljazeera.com/arabic",
    "skynewsarabia.com",
    "sabq.org",
    "alarabiya.net",
    "aleqt.com",
...
def process(df, event_date):
    ...
    # Override: priority sources get score 0 (highest priority)
    def is_priority(url):
        if not isinstance(url, str): return False
        url_lower = url.lower()
        return any(src in url_lower for src in PRIORITY_SOURCES)
    ...
    # Sort by source priority — best source first
    df = df.sort_values("src_priority")
    
    # Dedup step 1: one row per URL (most important — eliminates same article appearing multiple times)
    before = len(df)
    df = df.drop_duplicates(subset=["SOURCEURL"], keep="first")
    log.info(f"After URL dedup: {len(df):,} (removed {before-len(df):,})")
    
    # Dedup step 2: extract title after assigning source info
    df["title"] = df["SOURCEURL"].apply(extract_title)
    df["_title_norm"] = df["title"].str.lower().str.strip().str[:80]
    df["_domain"] = df["SOURCEURL"].str.extract(r"https?://(?:www\.)?([^/:]+)").fillna("")
    df["_dedup_key"] = df["_domain"] + "||" + df["_title_norm"]
    before2 = len(df)
    df = df.drop_duplicates(subset=["_dedup_key"], keep="first")
    df.drop(columns=["_title_norm","_domain","_dedup_key"], inplace=True)
    log.info(f"After title dedup: {len(df):,} (removed {before2-len(df):,})")
