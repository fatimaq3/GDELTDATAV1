...
# ── Priority sources (score 0) ────────────────────────────────────────────
PRIORITY_SOURCES = [
    "reuters.com", "apnews.com",
    "bbc.com", "bbc.co.uk", "arabic.bbc.co.uk",
    "aljazeera.com", "aljazeera.net",
    "theguardian.com", "middleeasteye.net",
    "france24.com", "arabnews.com",
    "alarabiya.net", "skynewsarabia.com",
    "sabq.org", "aleqt.com",
    "cnbc.com", "axios.com",
    "brookings.edu", "rand.org", "cfr.org",
    "carnegieendowment.org", "carnegie-mec.org",
    "csis.org", "gmfus.org", "atlanticcouncil.org",
    "heritage.org", "hoover.org", "hudson.org", "cato.org",
]
...
RULES = [
    ("Military", "Armed Conflict & Operations", [
        ...
        "iswap","hezbollah","killed in combat","died in battle",
        ...
        "air defense system","iron dome","patriot missile",
        ...
        "strikes","military strikes","us strikes","air strikes",
        "bombed","destroyed by","targeted by",
        "shot down","intercept drone","drones launched","drone attack",
    ]),
    ...
    ("Security", "Crime & Law Enforcement", [
        ...
        "gang warfare","gang shooting","riot","civil unrest","looting",
    ]),
    ("Security", "Threats & Coercion", [
        ...
        "bomb threat","chemical attack","chemical weapon",
    ...
    ("Energy", "Oil & Gas", [
        ...
        "oil tanker","gas tanker","tanker seized",
        ...
        "bahrain oil","oman oil","gulf energy","middle east oil",
    ...
    ("Energy", "Power & Renewables", [
        ...
        "electricity shortage","electricity prices",
        "green hydrogen","carbon neutral","net zero",
        "desalination plant","electricity grid attack","power station attack",
        "energy sanctions","energy embargo",
    ...
    ("Economic", "Trade & Finance", [
        ...
        "fiscal deficit","budget deficit","austerity","subsidy cut",
        "privatization","tadawul","remittances","tourism revenue",
        "non-oil gdp","economic diversification","fintech","startup",
    ...
    ("Economic", "Business & Industry", [
        ...
        "real estate","property market","ports","logistics",
        "container shipping","freight rates","labor market","job creation",
        "vision 2030","gulf cooperation","gcc trade",
    ]),
    ...
    ("Other", "Sports", [
        ...
        "homers","batting","pitcher","quarterback","touchdown",
    ]),
...
def classify_source(domain):
    ...
    if any(x in d for x in ["iheart","npr.org"]): return "Radio"
...
def process(df, event_date):
    ...
    # Priority sources get score 0
    def is_priority(url):
        if not isinstance(url, str): return False
        u = url.lower()
        return any(src in u for src in PRIORITY_SOURCES)
    df.loc[df["SOURCEURL"].apply(is_priority), "src_priority"] = 0

    # Dedup: sort by priority, keep best source per URL
    df = df.sort_values("src_priority")
    before = len(df)
    df = df.drop_duplicates(subset=["SOURCEURL", "EventRootCode"], keep="first")
    log.info(f"After dedup: {len(df):,} (removed {before-len(df):,})")
